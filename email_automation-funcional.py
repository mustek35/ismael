...
import re
import subprocess
from asistente_tecnico_api import asistente_tecnico_api
import psycopg2
...
from asistente_tecnico_api import consultar_api_acopio
...
def procesar_con_chatgpt(cuerpo_correo):
    ...
    if nuevo_thread_id:
        ...
        if mensaje_id:
            ...
            if resultado:
                ...
                centros = capturar_centros(cuerpo_correo)
                if centros:
                ...
                desconexion_detectada_en_correo = any(termino in cuerpo_correo.lower() for termino in [
                    "desconexión", "sin enlace", "sin visual", "ping", "problema de enlace", 
                    ...
                    "camara termal", "camara ptz", "ni visual"
                ])
                
                # Detectar si se menciona una desconexión o falta de visual en la respuesta del asistente
                desconexion_detectada_en_respuesta = any(termino in resultado.lower() for termino in [
                    "desconexión", "sin enlace", "sin visual", "ping", "problema de enlace", 
                    "cámaras sin visual", "no hay visual en central", "desconectado", "desconectada",
                    "No hay visual de camaras", "desconexion", "camaras en negro", 
                    "camara termal", "camara ptz", "ni visual"
...
def capturar_centros(cuerpo):
    """Extrae los nombres de centros de cultivo específicos del cuerpo del correo, organizados por empresa."""
    # Convertir el cuerpo del correo a minúsculas para hacer una búsqueda insensible a mayúsculas
    ...
    for empresa, centros in CENTROS_EMPRESAS.items():
        for centro in centros:
            ...
            # Si el centro aparece en el cuerpo del correo
            if centro_lower in cuerpo_lower:
...
def enviar_correo_en_hilo(respuesta_primaria, respuesta_tecnica, remitente, asunto, mensaje_original, remitente_fijo="cespinoza@orcatecnologia.cl", cc=None):
    msg = MIMEMultipart()
    msg['From'] = EMAIL
    ...
    # Crear el cuerpo del correo en HTML, combinando ambas respuestas
    html_body = f"""
    <p>{respuesta_primaria}</p>
    <p>{respuesta_tecnica}</p>
            ...
            if thread_id_principal:
                ...
                if mensaje_id_principal:
                    ...
                    if respuesta_primaria:
                        ...
                        centros_detectados, desconexion_detectada = procesar_con_chatgpt(cuerpo)
                        ...
                        if desconexion_detectada:
                            ...
                            if thread_id_tecnico:
                                ...
                                resultado_api = consultar_api_acopio(centros_detectados)
                                if resultado_api:
